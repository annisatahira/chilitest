	...
	function sendEmailVerificationCode() {
	  ...
	  if( $is_valid ) {
	  	...
	  	if( !$user_email ) {
	  		...
	  		$('#wcfm_membership_container .wcfm-message:not(.email_verification_message)').html( '<span class="wcicon-status-cancelled"></span>' + $('#user_email').data('required_message') ).addClass('wcfm-error').slideDown();
	  		$is_valid = false;
	  	} else {
		...
		function checkStoreSlug() {
			...
			if( $is_valid ) {
				...
				if( !$store_name ) {
					...
					$('#wcfm_membership_container .wcfm-message:not(.email_verification_message)').html( '<span class="wcicon-status-cancelled"></span>' + $('#store_name').data('required_message') ).addClass('wcfm-error').slideDown();
					$is_valid = false;
				} else {
	...
	$('#wcfm_membership_register_button').click(function(event) {
	  ...
	  if( $is_valid ) {
	  	...
	  	if( $password != $confirm_pwd ) {
	  		...
	  		$('#wcfm_membership_container .wcfm-message:not(.email_verification_message)').html( '<span class="wcicon-status-cancelled"></span>' + $('#passoword').data('mismatch_message') ).addClass('wcfm-error').slideDown();
	  		$is_valid = false;
	  	}
	  }
		...
		if( $is_valid ) {
			...
			$.post(wcfm_params.ajax_url, data, function(response) {
				if(response) {
					$response_json = $.parseJSON(response);
					if($response_json.status) {
						$('#wcfm_membership_registration_form .wcfm-message:not(.email_verification_message)').html('<span class="wcicon-status-completed"></span>' + $response_json.message).addClass('wcfm-success').slideDown( "slow", function() {
						  if( $response_json.redirect ) window.location = $response_json.redirect;	
						} );	
					} else {
						$('.wcfm-message').html('').removeClass('wcfm-success').slideUp();
						$('#wcfm_membership_registration_form .wcfm-message:not(.email_verification_message)').html('<span class="wcicon-status-cancelled"></span>' + $response_json.message).addClass('wcfm-error').slideDown();
					}
